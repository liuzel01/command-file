...
# 事实证明，函数ips还是写到 /etc/profile.d/sh.local... 因为，你用普通用户登录后再切换root的时候就会有、问题
if /usr/bin/fgrep -i "the func for getting ip of this Server." /etc/profile.d/sh.local; then
	exit 1
else
	cat >> /etc/profile.d/sh.local << salute
